...
对于上述这类问题应当如何解决？其实这种问题是版本相关的问题，推荐的解决办法是采用类似于乐观锁的版本控制。我们实现的缓存延迟删除方案，能够彻底解决这个问题。解决原理如下：
...
查询缓存时：
1. 如果缓存数据未被锁定，并且数据为空或被标记为删除
  a. 将数据锁定n秒，锁定者为请求的uuid
  b. 从数据库中获取数据
  c. 判断数据的锁定者是否为当前请求uuid，如果是，更新缓存；如果否，不更新缓存
2. 如果缓存中的数据被锁定，并且数据为空
  a. 睡眠1s后重新查询缓存
3. 其他数据情况
  a. 直接返回缓存中的数据（不管数据是否已经被标记删除）
...
在这个序列中，V发生在写入Vi之后，所以V等于Vi，保证了缓存的数据的最终一致性
