...
## 概述
本文提出的二阶消息，是首创的一种新架构，可以将本地消息表或者事务消息中，上百行的代码简化为大约五六行，极大地简化架构，提升开发效率，具备非常大的优势。

跨服务更新数据是应用开发常见的任务，如果一些关键数据对一致性的要求较高，而业务上又不需要支持回滚的话，那么传统方法会会采用本地消息表或者事务消息的方式来保证最终一致，而二阶段消息则是这种场景下dtm首创并推荐的解决方案。
...
## 普通消息
二阶消息不仅可以替换本地消息表方案，也能够替换普通消息方案。如果直接调用Submit，那么就与普通消息方案近似，但是提供了更灵活简单的接口。
...
## 小结
本文提出的二阶消息，接口简洁优雅，带来了比本地消息表和 Rocket 事务消息更优雅的架构，可以帮助大家更好的解决无需回滚的这一类数据一致性问题。
